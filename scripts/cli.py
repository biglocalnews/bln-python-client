...
from bln.exceptions import APIException
...
def authenticate(key: str):
    ...
    try:
        ...
        raise click.ClickException(
            "Invalid API key. Visit https://biglocalnews.org/#/manage_keys to confirm the key or create a new one."
        )
    else:
...
def init():
    ...
    # connect to an existing project
        ...
        role = [
            role
            for role in client.effectiveProjectRoles()
            if role["project"]["id"] == project_id
        ][0]["role"]
        ...
        if role == "VIEWER":
            click.echo(
                "You can download files from this project, but don't have permission to upload files. Contact a project administrator to get write permissions."
            )

    # Set the data directory
    project_root = Path.cwd()
    data_dir = click.prompt(
        "Specify a directory to store files",
        type=click.Path(),
        default=project_root.joinpath("data"),
    )
    ...
    metadata = {"project_id": project_id, "data_dir": str(data_dir)}
